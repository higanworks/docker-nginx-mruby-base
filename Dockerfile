...
RUN apk add --update openssl-dev git curl geoip-dev file wget \
  && apk add --virtual build-deps build-base ruby-rake bison \
  ...
  && ./nginx-build -d work -pcre -zlib -m /config/modules3rd.ini -c /config/configure.sh --clear \
